...
def get_raw_data(fname, test=False):
    train_raw = init_data(os.path.join(os.path.dirname(os.path.dirname(
        os.path.realpath(__file__))), fname), test=test)
    return train_raw
...
def process_word(word, word2vec, vocab, ivocab, word_vector_size, to_return="word2vec", silent=True, test=False, ft=False):
    if word2vec:
        if not word in word2vec:
            create_vector(word, word2vec, word_vector_size, silent)
    if not word in vocab:
        if (not test) and (not ft):
...
def process_context(context, word2vec, vocab, ivocab, embed_size, test=False, ft=False):
    result_temp = []
        ...
        for w in w_sep.findall(context):
            if w:
                result_temp.append(process_word(word=w,
                                                ...
                                                to_return="index", test=test, ft=ft))
    return result_temp


def process_input(data_raw, word2vec, vocab, ivocab, embed_size, cw, test=False, ft=False):
    if test or ft:
        q_obj = {'q': list(data_raw['input']), 'pred': None}

    inputs = data_raw['input'].apply(lambda x: process_context(x, word2vec, vocab, ivocab, embed_size, test=test, ft=ft))
    ...
    if (not test) and (not ft):
        ...
        mlb = joblib.load(os.path.join(cfg.DATA_DIR, 'mlb_cc_dmn_train.pkl'))
...
def load_input_data(config, cw, fname, test=False, ft=False):
    if test or ft:
        vocab = joblib.load(os.path.join(cfg.DATA_DIR, 'cc_vocabulary.pkl'))
        ivocab = joblib.load(os.path.join(cfg.DATA_DIR, 'cc_inverse_vocabulary.pkl'))
        ...
        if config.word2vec_init:
            ...
            word2vec = {}
    train_raw = get_raw_data(fname=fname, test=test)
    ...
    if test or ft:
        test_data = process_input(train_raw, word2vec, vocab, ivocab, config.embed_size, test=test, cw=cw, ft=ft)
    else:
    ...
    if test or ft:
        ...
        p = np.random.permutation(answers.shape[0])
        inputs, answers = inputs[p], answers[p]
    if (not test) and (not ft):
