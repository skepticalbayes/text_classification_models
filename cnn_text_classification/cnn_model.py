...
class Config(object):
    ...
    num_filters = 260
    train_mode = True
    filter_sizes = [1, 2, 3, 4, 5, 6, 10, 15]


class TextCNN:
    def __init__(self, filter_sizes,num_filters,num_classes, class_weights, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,vocab_size,embed_size,
        ...
        if not self.multi_label_flag:
            self.predictions = tf.argmax(self.possibility, 1, name="predictions")  # shape:[None,]
    ...
    def inference(self):
        ...
        for i,filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("convolution-pooling-%s" %filter_size):
                ...
                # ====>sparse_softmax_cross_entropy_with_logits. max-pooling.  value: A 4-D `Tensor` with shape `[batch, height, width, channels]
    ...
    def loss_multilabel(self,l2_lambda=0.000003,): #0.0001#this loss function is for multi-label classification
        with tf.name_scope("loss"):
            ...
            if self.class_weights is not None:
                loss = tf.reduce_mean((self.class_weights[:, 0] ** (1 - self.input_y_multilabel)) * \
                         (self.class_weights[:, 1] ** self.input_y_multilabel) * \
                         tf.nn.sigmoid_cross_entropy_with_logits(labels=self.input_y_multilabel,
                                                                 logits=self.logits))
            else:
                loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.input_y_multilabel, logits=self.logits));#losses=tf.nn.softmax_cross_entropy_with_logits(labels=self.input__y,logits=self.logits)
                #losses=-self.input_y_multilabel*tf.log(self.logits)-(1-self.input_y_multilabel)*tf.log(1-self.logits)
                print("sigmoid_cross_entropy_with_logits.losses:",loss) #shape=(?, 1999).
                # loss = tf.reduce_mean(losses)         #shape=().   average loss in the batch
            l2_losses = tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables() if 'bias' not in v.name]) * l2_lambda
            loss = loss+l2_losses
        return loss

    def loss(self, l2_lambda=0.000003):#0.001
        with tf.name_scope("loss"):
    ...
    def run_epoch(self, config, session, data, num_epoch=0, train_writer=None, train_op=None, verbose=2,
                  train=False, multilabel=True, device=None):
        dp = config.dropout
        if train_op is None:
            train_op = tf.no_op()
            dp = 1
        ...
        for step in range(total_steps):
            ...
            if multilabel:
                feed = {self.input_x: ip[index],
                        self.input_y_multilabel: a[index],
                        self.dropout_keep_prob: dp,
                        self.iter: step,
                        self.tst: not config.train_mode}
                if not device:
                    loss, _ = session.run(
                ...
                feed = {self.input_x: ip[index],
                        ...
                        self.tst: not config.train_mode}
                if not device:
                    acc, loss, _ = session.run(
                        [self.accuracy, self.loss_val, train_op], feed_dict=feed)
                else:
                    with tf.device('/device:GPU:{}'.format(device)):
                        acc, loss, _ = session.run(
                            [self.accuracy, self.loss_val, train_op], feed_dict=feed)
            ...
            if multilabel:
                ...
                if verbose and step % verbose == 0:
                    ...
                    sys.stdout.write('\r{} / {} : loss = {} accuracy = {} epoch_run_time = {}'.format(
                        step, total_steps, np.mean(total_loss), np.mean(Accuracy), end - start))
                    sys.stdout.flush()
